...
string makeMockFile::deleteAllBracketAfter1st(string line)
{
    try{
        string line_tmp=line;
        __LOG__("run",log,"notify");
...
int makeMockFile::countBracket(string line, char typeofBracket)
{
    try{
        ...
        for (size_t i = 0; i < len; i++)
        {
            ...
            if (line[i] == typeofBracket && !IsString)
                ++count;
...
int makeMockFile::checkisFunction(string line)
{
    try{
        ...
        //add to remove after signal {
        if(checkHasSpecialBracket!=-1)
        {
            line = getstring(line, 0, checkHasSpecialBracket);
        ...
        if(firstSpace > firstbreaket)
        {
            int findcolon_tmp=this->checkHasLetter(line,':',0, line.length());
            if(findcolon_tmp!=-1)
            {
                //check is constructor
                string className= getstring(line,0,findcolon_tmp-1);
                string constructor=getstring(line,findcolon_tmp+2,firstbreaket-1);
                __LOG__(className, log, "show value");
                __LOG__(constructor, log, "show value");
                if(className.compare(constructor)==0)
                {
             ...
             letterAfter1stSpace == -1 || line[letterAfter1stSpace] == '(' || firstbreaket == -1)
        {
            this->isfunction = 0;
            __LOG__(line, log, "invalid line");
            __LOG__(firstSpace, log, "invalid line");
            __LOG__(firstbreaket, log, "invalid line");
            __LOG__(firstSpace, log, "invalid line");
            __LOG__(letterAfter1stSpace, log, "invalid line");
            __LOG__(letterAfter1stSpace, log, "invalid line");
            __LOG__(firstbreaket, log, "invalid line");
            return 0;
        }
        ...
        else if (this->countBracketIn == this->countBracketOut)
        {
            ...
            //check whether any ) befor 1st bracket
            //            for(size_t i=0; i<checkFrom__; i++)
            //            {
            //                if(line[i]==')')
            //                {
            //                    if(i<(size_t)firstbreaket)
            //                    {
            //                        __LOG__(line, log, "invalid bracket!");
            //                        return 0;
            //                    }
            //                }
            //            }
        ...
        {
            __LOG__(line, log, "invalid number of bracket!");
            return 0;
        }
        if(secondbreaket<firstbreaket)
        {
            __LOG__(line, log, "invalid bracket!");
            return 0;
        }
        if(secondbreaket!=0)
        {
            ...
            if(checkHasSpecialBracket!=-1){
                line_PushToGetAfterBracket=getstring(line,0,checkHasSpecialBracket);
            }
            else
            {
            ...
            if(isnotSpaceAfterSecondBracket!=-1)
            {
                if(line[isnotSpaceAfterSecondBracket]!=';')
                {
                    ...
                    if(findComma!=-1 && findComma>isnotSpaceAfterSecondBracket)
                        this->AfterSecondBracket=getstring(line_PushToGetAfterBracket,isnotSpaceAfterSecondBracket,findComma);
                    else if(isnotSpaceAfterSecondBracket<(int)line_PushToGetAfterBracket.length() && findComma==-1)
                    {
                        ...
                        //                    __LOG__(AfterSecondBracket,log,"show value");
                        //                    __LOG__(line_PushToGetAfterBracket,log,"show value");
                        //                    __LOG__(isnotSpaceAfterSecondBracket,log,"show value");
                        //                    __LOG__(line_PushToGetAfterBracket.length(),log,"show value");
                    }
                }
            }
        }
        ...
        //check invalit before 1st bracket
        string invalidList="\",";
        for(size_t i=0; i<(size_t)firstbreaket;i++){
            int ss=checkHasLetter(invalidList,line[i],0,invalidList.length());
            if(ss!=-1)
            {
                __LOG__("invalid before 1st bracket",log,"notify");
                return 0;
            }
        }
        ...
        if(lineTemp=="NULL")
        {
            __LOG__("invalid line",log,"notify");
            return 0;
        }
        BracketInfor BracketInlineInfor=checkLevelOfBracket(line);
        if(secondbreaket!=BracketInlineInfor.pop&&BracketInlineInfor.pop!=0)
        {
            __LOG__(secondbreaket,log,"show infor");
            __LOG__(BracketInlineInfor.pop,log,"show infor");
            __LOG__("invalid the secondbreaket pos compare with BracketInlineInfor.pop",log, "notify");
            return 0;
        }
        ...
        else if (allspace != -1)
        {
            ...
            else
            {
                this->comman = 0;
                this->isfunction = 1;
                this->method0=1; // add 21.01.2021 the case nothing args
                __LOG__(this->containinbreaket,log,"show value");
                return 1;
            }
...
void makeMockFile::getagr()
{
    ...
    // line_ =(int a) -> (a)
    try{
        __LOG__("get args are starting",log,"notify");
        ...
        //        if(containInbracketAterRemove=="NULL")
        //        {
        //            __LOG__("invalid line",log,"notify");
        //            this->isfunction=0;
        //            this->args=" ";
        ...
        __LOG__(this->method0,log,"notify");
        this->args = dataout;
        __LOG__("get args are finished",log,"notify");
    }
    catch(exception &e)
    {
        __LOG__(e.what(),log,"show exception");
    }
...
void makeMockFile::makeFileHeader()
{
    ...
    this->fbout << "\t\t\t/*******************************************\n";
    this->fbout << "\t\t\t********************************************\n";
    this->fbout << "\t\t\t*****GENERATED BY LGE DCV HN GMOCK 2.0******\n";
    this->fbout << "\t\t\t********************************************\n";
    this->fbout << "\t\t\t*******************************************/\n";
    this->fbout << "class ";
...
{
    try{
        __LOG__("writing is starting",log,"notity");
...
void makeMockFile::checktypeofFuntion()
{
    try{
        ...
        this->deleteSpaceBtwWords(this->namefunction);
        string invalidKey[] = {"break", "else", "else if", "auto", "case", "typedef", "return", "}else", "throw", "else throw"};
        ...
        __LOG__("exit function check typeofFunction",log,"notify");
    }
    catch(exception &e)
    {
        __LOG__(e.what(),log,"exception");
    }
}
void makeMockFile::readFilename()
{
    ...
    if (this->fb.fail()){
        ...
        __LOG__("read failed",log,"noftify");}
    string line;
    ...
    while (getline(this->fb, line))
    {
        try{
            ...
            {
                //                if(this->countBracketIn!=countBracketOut)
                //                {
                //                    this->countBracketIn=0;
                //                    this->countBracketOut=0;
                //                     __LOG__(line, log, "invalid line reset numberof bracket");
                //                    continue;
                //                }
                    ...
                    if (this->countBracketIn==this->countBracketOut)
                    {
                        ...
                        if(checkIsFuncionResultInMultipLine==1)
                        {
                            ...
                            if(!this->ismakedUTSheader)
                            {
                                ...
                                this->makeHeaderfileForUTscript();
                                this->ismakedUTSheader=1;
                                __LOG__("make header file for UT script is finished",log," request make header");
                            }
                        }
...
void makeMockFile::makeHeaderfileForUTscript()
{
    ...
    {
        //        cout << "cannot gen UT script" << endl;
        __LOG__("cannot gen UT script",log,"notify");
        return;
    }
    ...
    this->fbUTtest << "\t\t\t/*******************************************\n";
    this->fbUTtest << "\t\t\t********************************************\n";
    this->fbUTtest << "\t\t\t*****GENERATED BY LGE DCV HN GMOCK 2.0******\n";
    this->fbUTtest << "\t\t\t********************************************\n";
    this->fbUTtest << "\t\t\t*******************************************/\n";
...
{
    try{
        __LOG__(str+"delete free space is starting",log,"notify");
        ...
        __LOG__(str+"delete free space is finished",log,"notify");
    }
    catch(exception &e)
    {
        __LOG__(e.what(),log,"exception");
